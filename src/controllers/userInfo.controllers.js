...
export const userInfo = async (req, res) => {
    upload.single("image")(req, res, async (err) => {
        if (err) {
            console.error("Upload Error:", err); // Log Cloudinary error
            return res.status(400).json({
                success: false,
                message: err.message,
            });
        ...
        const abcd = req.file ? req.file : null;
        console.log('abcd', abcd);
        try {
            if (!navText || !designation || !userName || !shortDesc || !email || !imageUrl) {
                return res.status(400).json({
                    success: false,
                    message: "All fields are required",
                });
...
export const updateUserInfo = async (req, res) => {
    ...
    upload.single("image")(req, res, async (err) => {
        ...
        try {
            ...
            if (req.file) {
                user.image = req.file.path;  // New Cloudinary URL
                user.imageName = req.file.public_id;
...
export const deleteUserInfo = async (req, res) => {
    ...
    try {
        ...
        if (user.imageName) {
            await cloudinary.uploader.destroy(user.imageName); // Delete from Cloudinary
        }
